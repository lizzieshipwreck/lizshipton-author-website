export const thalassic_na_image = "na thalassic complete";
export const thalassic_na_alt = "Covers for the complete Thalassic New Adult series";
export const thalassic_na_amazon_link = "https://amzn.to/3PBZR62";
export const thalassic_na_shop_link = "https://shop.lizshipton.com/collection/paperbacks";
export const thalassic_na_cta_text = "Shop Now";
export const thalassic_na_cta_mobile_text = "Shop";
export const thalassic_na_button_color = "Soul";
export const savage_na_link = "https://shop.lizshipton.com/b/XDJip";
export const savage_promo = "savage vs zahara";
export const savage_button_color = "Savage";
export const savage_button_text = "Order Now";
export const savage_mobile_text = "Order";
export const savage_promo_alt = "Savage vs. Zahara promo image";
export const salt_tropes_alt = "Listed tropes for Salt";
export const salt_tropes = "salt tropes";
export const salt_na_link = "https://shop.lizshipton.com/b/ija7x";
export const salt_button_color = "Salt";
export const salt_button_text = "Read Now";
export const salt_mobile_text = "Read";